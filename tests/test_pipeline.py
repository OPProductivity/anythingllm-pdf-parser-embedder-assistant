...
class PipelineCoreTests(unittest.TestCase):
    ...
    def test_cli_run_root_does_not_merge_two_launches_with_the_same_timestamp(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            first = pipeline.create_fresh_cli_run_root(base, timestamp="20260820-120000")
            second = pipeline.create_fresh_cli_run_root(base, timestamp="20260820-120000")

        self.assertEqual(first.name, "run-20260820-120000")
        self.assertEqual(second.name, "run-20260820-120000-1")
        self.assertNotEqual(first, second)

    def test_edge_case_run_root_uses_the_same_exclusive_allocator(self):
        import rag_pdf_gradio_app as app

        fixed_now = app.datetime(2026, 8, 20, 12, 0, 0)
        with tempfile.TemporaryDirectory() as temp_dir:
            with mock.patch.object(app, "datetime", SimpleNamespace(now=lambda: fixed_now)):
                first = app.create_fresh_automatic_run_root(temp_dir, prefix="edge-case-run")
                second = app.create_fresh_automatic_run_root(temp_dir, prefix="edge-case-run")

        self.assertEqual(first.name, "edge-case-run-20260820-120000")
        self.assertEqual(second.name, "edge-case-run-20260820-120000-2")

    def test_windows_executable_candidates_do_not_fall_back_to_relative_paths(self):
        ...
        with mock.patch.dict(
            ...
            self.assertEqual(pipeline.default_anythingllm_desktop_executable_candidates(), [])
        with mock.patch.dict(
            rag_pdf_tools.os.environ,
            {"LOCALAPPDATA": "", "ProgramFiles": "", "ProgramFiles(x86)": ""},
            clear=False,
        ):
            self.assertEqual(rag_pdf_tools.default_tesseract_executable_candidates(), [])
        with mock.patch.dict(
            pipeline.os.environ,
            {"LOCALAPPDATA": "relative-local", "ProgramFiles": "relative-program-files", "ProgramFiles(x86)": ""},
            clear=False,
        ):
            self.assertEqual(pipeline.default_anythingllm_desktop_executable_candidates(), [])
        with mock.patch.dict(
    ...
    def test_observer_log_rotation_does_not_abort_snapshot(self):
        import rag_pdf_gradio_app as app

        with tempfile.TemporaryDirectory() as temp_dir:
            log_path = Path(temp_dir) / "logs" / "backend-rotating.log"
            log_path.parent.mkdir()
            log_path.write_text("workspace-a indexed\n", encoding="utf-8")
            original_stat = Path.stat

            def disappearing_log_stat(path, *args, **kwargs):
                if path == log_path:
                    raise FileNotFoundError("log rotated")
                return original_stat(path, *args, **kwargs)

            with mock.patch.object(Path, "stat", disappearing_log_stat):
                snapshot = app._observer_tail_log_lines(temp_dir, "workspace-a")

        self.assertEqual(snapshot, {"file": "", "modified_epoch": 0.0, "matches": []})
    ...
    def test_confirmed_new_document_workspace_is_created_only_when_run_is_confirmed(self):
        ...
        original_compatibility = app.automatic_native_mutation_compatibility_report
        created = []
        captured = {}
        try:
            ...
            app.automatic_native_mutation_compatibility_report = lambda *_args, **_kwargs: {
                "status": "pass",
            ...
            app.automatic_native_mutation_compatibility_report = original_compatibility

    def test_unqualified_contract_stops_before_new_workspace_mutation(self):
        import rag_pdf_gradio_app as app
        ...
        with mock.patch.object(
            app,
            "automatic_native_mutation_compatibility_report",
            return_value={
                "status": "blocked",
                "reason": "required_native_mutation_capability_not_qualified",
                "required_capabilities": ["can_create_workspace"],
                "blocked_capabilities": ["can_create_workspace"],
                "characterization": {},
            },
        ), mock.patch.object(app, "create_new_document_workspace") as create_workspace:
            response = app.run_automatic_from_confirmation(settings)

        create_workspace.assert_not_called()
        self.assertIn("No workspace was created", str(response))
    ...
    def test_default_upload_layout_is_visible_to_the_desktop_documents_drawer(self):
        ...
        self.assertFalse(updates[16]["value"])
    ...
    def test_background_reconciliation_preserves_a_valid_workspace_and_stays_observational(self):
        ...
        try:
            ...
            # Passive timer observations must never write a possibly stale
            # workspace value back over browser-owned form state.
    ...
    def test_background_reconciliation_does_not_replace_a_missing_browser_selection(self):
        import rag_pdf_gradio_app as app
    ...
    def test_automatic_completion_requires_workspace_evidence_for_green_status(self):
        ...
        self.assertEqual(concurrent_completion["state"], "successful")
        self.assertEqual(concurrent_completion["diagnostic_code"], "AUTO-STORAGE-WRITE-ACTIVE-001")
    ...
    def test_stage_anomaly_detection_flags_only_material_historical_outlier(self):
        ...
        def timing_row(key, queue_seconds):
            return {
                "run_key": key,
                "mode": app.MODE_NATIVE_UPLOAD_LABEL,
                ...
                "actual_records": 100,
                "batch_seconds": [queue_seconds],
                "batch_measurements": [{"state": "accepted", "records": 100}],
                "document_count": 2,
                "page_count": 100,
                "document_timing": [],
            }
        ...
        history = [
            timing_row(f"prior-{index}", seconds)
            for index, seconds in enumerate((180, 200, 220, 240, 260), start=1)
        ]
        current = timing_row("current", 1400)

        anomalies = app.detect_unusually_slow_stages(current, history)

        self.assertEqual([item["key"] for item in anomalies], ["anythingllm_queue"])
        message = app.unusually_slow_stage_message(anomalies)
        self.assertIn("AnythingLLM embedding and verification: 23m 20s", message)
        self.assertIn("historical median 2.2s", message)

    def test_stage_anomaly_detection_does_not_flag_normal_variation_or_short_stage(self):
        import rag_pdf_gradio_app as app

        history = []
        for index, seconds in enumerate((180, 200, 220, 240), start=1):
            history.append({
                "run_key": f"prior-{index}",
                "mode": app.MODE_NATIVE_UPLOAD_LABEL,
                "embedding_timing_lane": "cloud:openrouter",
                "native_upload_representation": "page_parents",
                "segment_mode": "page_limit",
        ...
        current = dict(history[-1], run_key="current", batch_seconds=[300])
    ...
    def test_ocr_withheld_terminal_phase_never_claims_searchable_vectors(self):
        ...
        self.assertIn("not processed", phase)
        self.assertNotIn("vectors verified", phase.casefold())

    def test_upload_failure_is_not_masked_by_a_separate_ocr_review_hold(self):
        import rag_pdf_gradio_app as app
        ...
        completion = app.automatic_completion([
            {
                "pdf": "needs-ocr.pdf",
                "api_upload_status": "skipped_needs_ocr_review",
                "api_upload_warning": "AnythingLLM upload was withheld because OCR needs review.",
            },
            {
                "pdf": "submission-failed.pdf",
                "api_upload_status": "error",
                "api_upload_error": "AnythingLLM returned HTTP 500.",
            },
        ], True)
    ...
    def test_ocr_hold_reports_confirmed_siblings_without_claiming_the_hold_uploaded(self):
        ...
        self.assertEqual(completion["not_processed_pdf_count"], 1)
        self.assertIn("1 PDF(s) not processed", completion["message"])
        self.assertNotIn("need review", completion["message"])
        ...
        self.assertEqual(
            app.automatic_completion_phase(completion, True),
            "Eligible PDFs ready — remaining PDFs not processed",
        )
        self.assertEqual(app.automatic_terminal_confirmed_fraction(completion), 1.0)
        button = app.automatic_completion_button_state(completion)
        self.assertEqual(button["value"], "Completed — 1 PDF not processed")

    def test_terminal_status_message_uses_the_persistent_readable_lane_and_hides_internal_code(self):
        import rag_pdf_gradio_app as app

        rendered = app.automatic_live_status_html({
            "state": "warning",
            "phase": "Eligible PDFs ready — remaining PDFs not processed",
            "details": "Two PDFs were not processed because their extraction candidates disagreed materially.",
            "completion_code": "AUTO-OCR-REVIEW-PARTIAL-001",
            "confirmed_fraction": 1.0,
            "started_epoch": 100.0,
            "last_activity_epoch": 120.0,
        })

        self.assertIn('class="automatic-run-progress-details"', rendered)
        self.assertNotIn("automatic-run-terminal-details", rendered)
    ...
    def test_grouped_upload_status_keeps_queue_and_verification_lanes_together(self):
        ...
        display = {}
        queue = app.combined_grouped_upload_status_details(
            display,
            "AnythingLLM queue group 5 of 6: embedding 198/340 records",
            {"timing_event": "queue_progress", "source_window_index": 5},
        )
    ...
    def test_grouped_upload_status_discards_prior_source_verification(self):
        ...
        display = {}
        app.combined_grouped_upload_status_details(
            display,
            "Checking workspace attachment: source 4",
            {"timing_event": "exact_vector_observation", "source_window_index": 4},
        )
        next_source = app.combined_grouped_upload_status_details(
            display,
            "AnythingLLM queue group 5: embedding 1/20 records",
            {"timing_event": "queue_progress", "source_window_index": 5},
        )

        self.assertIn("queue group 5", next_source)
        self.assertNotIn("source 4", next_source)

    def test_terminal_ocr_hold_records_workflow_completion_separately_from_upload(self):
        import rag_pdf_gradio_app as app
        ...
        try:
            ...
            completion = {
                "state": "warning",
                "code": "AUTO-OCR-REVIEW-PARTIAL-001",
                "not_processed_pdf_count": 2,
            }
            updated = app.update_live_automatic_run_status(
                state="warning",
                phase="Eligible PDFs ready — remaining PDFs not processed",
                details="2 PDFs not processed.",
                completion_code=completion["code"],
                not_processed_pdf_count=2,
                confirmed_fraction=app.automatic_terminal_confirmed_fraction(completion),
            )
        finally:
            app.LIVE_AUTOMATIC_RUN_STATUS = original

        self.assertEqual(updated["confirmed_fraction"], 1.0)
    ...
    def test_confirmed_dispatch_passes_expected_seconds_once_by_keyword(self):
        ...
        self.assertEqual(
            captured["kwargs"]["download_segments_folder"],
            f"value-{app.AUTOMATIC_RUN_FIELDS.index('download_segments_folder')}",
        )
    ...
    def test_overlapping_recovery_schedulers_reserve_only_one_attempt(self):
        """A concurrent callback must observe the first durable reservation."""
        import rag_pdf_gradio_app as app

        original_write = app._write_automatic_run_json
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir) / "app-run-interrupted"
            root.mkdir()
            attempt_path = root / app.AUTOMATIC_RUN_RECOVERY_ATTEMPT

            def delayed_attempt_write(path, payload):
            ...
            def schedule(reason):
                try:
                    results.append(app.schedule_automatic_recovery(root, reason=reason))
                except Exception as exc:  # Surface thread failures in the test.
                    errors.append(exc)

            try:
                app._write_automatic_run_json = delayed_attempt_write
                app.recover_automatic_run = lambda *_args, **_kwargs: None
                first = threading.Thread(target=schedule, args=("first",))
                first.start()
                self.assertTrue(first_write_entered.wait(timeout=2))
                second = threading.Thread(target=schedule, args=("second",))
                second.start()
                release_first_write.set()
                first.join(timeout=2)
                second.join(timeout=2)
            finally:
                app._write_automatic_run_json = original_write
    ...
    def test_overlapping_timing_backfills_do_not_learn_one_run_twice(self):
        import rag_pdf_gradio_app as app
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            summary_path = output_root / "r-fixture" / "document" / "run-summary.json"
            summary_path.parent.mkdir(parents=True)
            summary_path.write_text(json.dumps({
                "total_pipeline_seconds": 20,
                "pdf_page_count": 2,
            }), encoding="utf-8")
            (summary_path.parent.parent / "ingestion-terminal-record.json").write_text(
            ...
            def delayed_append(path, row):
                nonlocal append_count
                with append_count_lock:
                    append_count += 1
                    first = append_count == 1
                if first:
                    first_append_entered.set()
                    self.assertTrue(release_first_append.wait(timeout=2))
                return original_append(path, row)

            def backfill():
                try:
                    results.append(app.ensure_timing_model_backfill())
                except Exception as exc:
                    errors.append(exc)

            try:
                app._append_timing_jsonl = delayed_append
                first = threading.Thread(target=backfill)
                first.start()
                self.assertTrue(first_append_entered.wait(timeout=2))
                second = threading.Thread(target=backfill)
                second.start()
                ...
                rows = [
                    json.loads(line)
                    for line in app.TIMING_MODEL_RUNS_PATH.read_text(encoding="utf-8").splitlines()
                    if line.strip()
                ]
            finally:
        ...
        self.assertEqual(rows[0]["run_key"], str(summary_path.parent.parent))
    ...
    def test_lean_ready_retention_keeps_root_text_and_page_bounded_segment_files(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            summary = {
                ...
                "temporary_workspace_validation_status": "complete",
                "temporary_workspace_validation_post_upload_status": "pass_with_missing_workspace_document_records",
                "temporary_workspace_validation_runtime_status": "pass",
                "temporary_workspace_validation_retention_status": "cleaned_up",
                "temporary_workspace_validation_cleanup_status": "deleted",
                "temporary_workspace_validation_chunk_survival_flag": "preserved",
                "temporary_workspace_validation_chunk_survival_ratio": 1.0,
            ...
            self.assertEqual(compact["verification_receipt"]["temporary_canary"], {
                "status": "complete",
                "post_upload_status": "pass_with_missing_workspace_document_records",
                "runtime_status": "pass",
                "retention_status": "cleaned_up",
                "cleanup_status": "deleted",
                "chunk_survival_flag": "preserved",
    ...
    def test_deferred_batch_lean_retention_compacts_only_after_exact_vector_proof(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            selected = root / "selected"
            selected.mkdir()
            ...
            manifest.write_text(
                '\n'.join([
                    json.dumps({"pdf_page": 1, "text": "First selected segment."}),
                    json.dumps({"pdf_page": 2, "text": "Second selected segment."}),
                ]) + '\n',
                encoding="utf-8",
            )
            (root / "metadata-api").mkdir()
            (root / "metadata-api" / "page-parent-upload-plan.csv").write_text("plan", encoding="utf-8")
            (root / "source-profile.json").write_text(
                json.dumps({"source_file": "C:/Example.pdf", "filename": "Example.pdf", "source_sha256": "f" * 64}),
                encoding="utf-8",
            )
            summary = {
            ...
            self.assertTrue(result["applied"])
            self.assertEqual(result["policy"], "lean_success_after_shared_batch_v1")
            self.assertFalse(selected.exists())
            self.assertFalse((root / "metadata-api").exists())
    ...
    def test_deferred_batch_lean_retention_keeps_all_artifacts_without_exact_vector_proof(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            summary = {
                ...
                "anythingllm_runtime_validation_status": "deferred_after_exact_vector_proof",
                "batch_upload_result": {"searchability_proven": False},
                "lean_retention": {"deferred": True, "reason": "awaiting_shared_automatic_batch_upload"},
            }
            ...
            self.assertFalse(result["applied"])
            self.assertEqual(result["reason"], "exact_batch_vector_proof_missing")
            self.assertTrue(parsed.exists())
            self.assertTrue(manifest.exists())
            self.assertTrue((root / "metadata-api").exists())
    ...
    def test_progress_duration_uses_a_stable_full_width_host(self):
        import rag_pdf_gradio_app as app

        self.assertIn(".automatic-run-timing-host", app.APP_CSS)
        self.assertIn("width: 100% !important", app.APP_CSS)
        self.assertIn("min-height: 1.25em", app.APP_CSS)
    ...
    def test_exact_batch_plan_prices_only_fully_cached_source_windows(self):
        import rag_pdf_gradio_app as app

        all_sources = app.confirmed_prequeue_cache_eta_seconds(
            10_000,
            191,
            fresh_provider_requests=621,
            provider_request_seconds=1.72,
            features={"document_count": 21},
            cached_attachment_records=595,
            remaining_source_windows=21,
            cache_attachment_prior={"record_seconds": 0.08, "source_seconds": 3.15},
        )
    ...
    def test_exact_batch_plan_drops_provisional_ocr_surcharge_from_remaining_eta(self):
        ...
        # 4,818 seconds mirrors an OCR-inflated opening estimate. Once all
        # preparation is represented by elapsed time and the batch identities
        # are exact, only the unstarted fresh/cache work belongs in the ETA.
        exact_plan = app.confirmed_batch_execution_plan_eta_seconds(
            4_818,
            191,
            fresh_provider_requests=621,
    ...
    def test_native_boundary_policy_refreshes_eta_with_derived_controls(self):
        ...
        # ``refresh_automatic_run_estimate`` now carries Custom Range group
        # sizes between segmentation mode and target length.
        self.assertEqual(observed[0][6], "")
        self.assertEqual(observed[0][7], "750")
        self.assertEqual(observed[0][8], "512")
        self.assertEqual(observed[0][9], "0")
    ...
    def test_create_document_workspace_selects_the_new_workspace(self):
        ...
        self.assertIn("Example PDF", update["choices"][0][0])
        self.assertNotIn("PDF —", update["choices"][0][0])
    ...
    def test_reconciled_failure_preserves_specific_workspace_code(self):
        ...
        self.assertIn("workspace setup", outputs[7]["value"])
        self.assertNotIn("AUTO-WORKSPACE-004", outputs[7]["value"])
    ...
    def test_workspace_batch_corroborates_filename_surname_only_with_verified_person(self):
        import rag_pdf_gradio_app as app

        identities = {
            "Marx-Capital.pdf": {
                "filename": "Marx-Capital.pdf", "label": "Unknown", "kind": "unknown",
            },
            "Marx-Manuscripts.pdf": {
                "filename": "Marx-Manuscripts.pdf", "label": "Marx", "kind": "person",
            },
            "Politics-of-Virtue.pdf": {
                "filename": "Politics-of-Virtue.pdf", "label": "Unknown", "kind": "unknown",
            },
        }
        with mock.patch.object(
            app,
            ...
            plan = app.workspace_source_identity_plan(list(identities))

        self.assertEqual([row["label"] for row in plan], ["Marx", "Marx", "Unknown"])
        self.assertIn("corroborated", plan[0]["provenance"])

        underscore_plan = app.corroborate_workspace_source_identities([
            {
                "filename": "Marx_German_Ideology_Chapter_3.pdf",
                "label": "Unknown",
                "kind": "unknown",
            },
            {
                "filename": "Marx-Manuscripts.pdf",
                "label": "Marx",
                "kind": "person",
            },
        ])
        self.assertEqual(underscore_plan[0]["label"], "Marx")
    ...
    def test_filename_leading_author_conventions_beat_weak_title_evidence(self):
        ...
        self.assertEqual(
            (multiple["author"], multiple["source"]),
    ...
    def test_vector_liveness_ignores_broader_identity_set_for_current_source(self):
        import rag_pdf_gradio_app as app

        report = {
            "matching_vector_rows": 12,
            "lancedb_matching_rows": 12,
            "current_upload_document_vector_count": 2,
            # This includes ten prior source identities and must not advance
            # the current two-record source window.
            "observed_chunk_source_count": 12,
        }
        self.assertEqual(
            app.current_source_vector_progress_count(report, expected_records=2),
            2,
        )

    def test_grouped_source_progress_ignores_workspace_wide_vector_total(self):
        import rag_pdf_gradio_app as app
    ...
    def test_live_run_status_renders_batch_completion_separately_from_queue_evidence(self):
        ...
        self.assertIn("PDFs completed: 7/8", rendered)
    ...
    def test_live_run_status_distinguishes_document_and_page_progress(self):
        ...
        self.assertIn("PDFs completed: 0/16", rendered)
    ...
    def test_large_batch_history_needs_two_fresh_cohorts_and_one_thousand_records(self):
        import rag_pdf_gradio_app as app
        ...
        # One cohort is deliberately insufficient even when it is large.
        self.assertEqual(
            app.timing_model_large_batch_provider_request_prior_seconds(
                features, [matching_row()], 6.0
            )[:2],
            (6.0, 1),
        )
        ...
        self.assertEqual(
            app.timing_model_large_batch_provider_request_prior_seconds(
                features, [matching_row(cached=1), matching_row()], 6.0
            )[:2],
            (6.0, 1),
        )
        # Two independent 1,000-record fresh cohorts permit refinement. The
        # queue observation already carries an upper-quartile 25% margin and
        # still cannot replace the classic prior completely.
        ...
        self.assertEqual(prior, 2.4375)
        self.assertGreaterEqual(prior, app.TIMING_MODEL_LARGE_BATCH_MIN_SECONDS)
    ...
    def test_batch_queue_forecast_does_not_price_proved_cache_as_provider_work(self):
        ...
        self.assertEqual(plan["cached_documents"], 17)
        self.assertEqual(plan["partially_cached_documents"], 4)
    ...
    def test_mature_queue_reprice_moves_the_eta_in_bounded_readable_steps(self):
        ...
        # single UI repaint.
        self.assertEqual(app.bounded_queue_eta_reprice(120, 2_640), 126)
        self.assertEqual(app.bounded_queue_eta_reprice(120, 20), 90)

    def test_batch_queue_maturity_accumulates_across_small_source_windows(self):
        import rag_pdf_gradio_app as app

        context = {"prepared_records": 40}
        first = app.observe_batch_queue_forecast(
            context,
            {
                ...
                "queue_records": 8,
                "desktop_queue_completed": 8,
                "desktop_queue_events_observed": 8,
                "desktop_queue_records_per_minute": 12,
            },
            elapsed_seconds=60,
            provider_request_seconds_prior=3.0,
        )
        self.assertFalse(app.batch_queue_forecast_is_mature(first))
        second = app.observe_batch_queue_forecast(
            context,
            {
                "source_path": "second.pdf",
                ...
                "source_window_total": 3,
                "queue_records": 6,
                "desktop_queue_completed": 4,
                "desktop_queue_events_observed": 4,
                "desktop_queue_records_per_minute": 2,
            },
            elapsed_seconds=180,
            provider_request_seconds_prior=3.0,
        )
        self.assertEqual(second["completed_records"], 12)
        self.assertEqual(second["observed_event_samples"], 12)
        self.assertTrue(app.batch_queue_forecast_is_mature(second))
    ...
    def test_upward_queue_reprice_requires_cross_source_consensus_and_is_small(self):
        ...
        forecasts = [3_000, 3_020, 3_040, 3_060, 3_080]
        self.assertIsNone(
            app.stable_queue_eta_reprice(2_000, forecasts, observed_windows=1)
        )
        increased = app.stable_queue_eta_reprice(
            2_000, forecasts, observed_windows=2
        )
        self.assertEqual(increased["raw_forecast_seconds"], 3_040)
        self.assertEqual(increased["expected_seconds"], 2_090)
        self.assertIsNone(
            app.stable_queue_eta_reprice(
                1_000, [1_500, 700, 1_520, 800, 1_540], observed_windows=3
            )
        )
        short_run = app.stable_queue_eta_reprice(
            378,
            [900, 920, 940, 960, 980],
            observed_windows=2,
        )
    ...
    def test_partial_indexing_takes_priority_over_generic_submission_timeout(self):
        ...
        self.assertIn("not-fully-indexed.pdf", completion["message"])
    ...
    def test_fresh_selection_resets_per_run_controls_to_defaults(self):
        ...
        updates = app.reset_automatic_run_settings_to_defaults()
        self.assertEqual(len(updates), 45)
        self.assertEqual(updates[0]["value"], "")  # document label
        self.assertEqual(updates[6]["value"], str(app.AUTO_OUTPUT_DIR))
        self.assertEqual(updates[8]["value"], "")  # visible API-key field
        self.assertEqual(updates[9]["value"], app.INITIAL_WORKSPACE_VALUE)
        self.assertEqual(updates[10]["value"], "")  # new workspace name
        self.assertEqual(updates[11], "")  # generated-name auto state
        self.assertEqual(updates[28]["visible"], False)  # Custom Range controls
        self.assertEqual(updates[29]["value"], "Automatic")
        self.assertEqual(updates[38]["value"], False)  # do not retain apply-before-run
        self.assertEqual(updates[42]["value"], "")  # validation phrases
    ...
    def test_reserved_preparing_run_blocks_late_selection_reset_callbacks(self):
        """Startup is in-flight before the worker changes status to running."""
        import rag_pdf_gradio_app as app

        original_status = app.LIVE_AUTOMATIC_RUN_STATUS
        reserved = {"state": "preparing", "run_root": "reserved-run", "phase": "Starting"}
        try:
            app.LIVE_AUTOMATIC_RUN_STATUS = dict(reserved)
            presentation = app.reset_automatic_run_presentation(["next.pdf"], [])
            settings = app.reset_automatic_run_settings_to_defaults()
            observed_status = dict(app.LIVE_AUTOMATIC_RUN_STATUS)
        finally:
            app.LIVE_AUTOMATIC_RUN_STATUS = original_status
    ...
    def test_fresh_selection_history_defaults_match_the_reset_controls(self):
        ...
        field_to_update = {
            ...
            "apply_batch_metadata_overrides": 4,
            "mode": 5,
            "output_root_override": 6,
            "api_url": 7,
            "api_key": 8,
            "workspace_slug": 9,
            "native_upload_scope": 12,
            "native_upload_custom_range": 13,
            "native_metadata_mode": 15,
            "anythingllm_create_document_folders": 16,
            "anythingllm_document_folder_name": 17,
            "existing_workspace_duplicate_policy": 18,
            "local_check_mode": 19,
            "custom_ollama_model": 20,
            "ollama_url": 21,
            "vector_audit_scope": 22,
            "deep_extraction": 23,
            "include_front_matter": 24,
            "include_back_matter": 25,
            "segment_mode": 26,
            "custom_page_group_sizes": 27,
            # Index 28 is the Custom Range visibility container, which has
            # no value. The remaining mounted controls follow it.
            "backend_mode": 29,
            "first_page_override": 30,
            "end_page_override": 31,
            "target_passage_length": 33,
            "page_preserve_ceiling": 34,
            "inherit_anythingllm_settings": 35,
            "anythingllm_chunk_size": 36,
            "anythingllm_chunk_overlap": 37,
            "auto_apply_recommended_settings": 38,
            "download_full_folder": 39,
            "download_segments_folder": 40,
            "advanced_end_section_names": 41,
            "automatic_validation_phrases": 42,
            "unstructured_strategy": 43,
            "generate_inline_fallback": 44,
        }
    ...
    def test_saved_defaults_refresh_only_an_idle_automatic_form(self):
        ...
        original_status = app.LIVE_AUTOMATIC_RUN_STATUS
        expected = tuple({"value": f"saved-{index}"} for index in range(45))
        try:
    ...
    def test_running_estimate_counts_down_and_uses_a_stable_queue_tail_after_overrun(self):
        ...
        observed_forecast = app.automatic_run_timing_html(
            expected_seconds=138,
            state="running",
            started_epoch=100.0,
            now=240.0,
            ...
            completed_units=1,
            total_units=13,
            queue_forecast_expected_seconds=600,
        )
        ...
        self.assertIn("Est: ~07m40s", observed_forecast)
    ...
    def test_terminal_status_releases_process_local_cancellation_root(self):
        import rag_pdf_gradio_app as app
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                self.assertTrue(app.request_automatic_run_cancellation(run_root))
                self.assertIn(str(run_root), app.CANCELLED_AUTOMATIC_RUN_ROOTS)
                ...
                self.assertNotIn(str(run_root), app.CANCELLED_AUTOMATIC_RUN_ROOTS)
    ...
    def test_old_worker_cleanup_preserves_a_replacement_worker_marker(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            run_root = Path(tmpdir) / "app-run-restarted"
            run_root.mkdir()
            marker = run_root / app.AUTOMATIC_RUN_WORKER_MARKER
            marker.write_text(
                json.dumps(
            ...
            removed = app.remove_owned_automatic_worker_marker(
                run_root,
                SimpleNamespace(pid=9001),
            )
            successor = app.active_automatic_run_worker(run_root)

        self.assertFalse(removed)
        self.assertEqual(successor["pid"], 9002)

    def test_worker_registration_failure_stops_the_untracked_child(self):
        import rag_pdf_gradio_app as app

        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir) / "app-run"
            output = root / "output"
            process = mock.Mock(pid=9003)
            process.poll.return_value = None
            process.wait.return_value = 0
            writes = []

            def write_then_fail_marker(*args, **_kwargs):
                writes.append(args[0])
                if len(writes) == 2:
                    raise OSError("worker marker storage unavailable")

            args = SimpleNamespace()
            with (
                mock.patch.object(app, "_write_automatic_run_json", side_effect=write_then_fail_marker),
                mock.patch.object(app.subprocess, "Popen", return_value=process),
                mock.patch.object(app.subprocess, "run", return_value=SimpleNamespace(returncode=0)) as taskkill,
            ):
                with self.assertRaisesRegex(OSError, "worker marker storage unavailable"):
                    app.execute_automatic_preparation_in_worker(
                        "source.pdf",
                        output,
                        args,
                        root,
    ...
    def test_worker_replacement_requires_the_original_process_to_exit(self):
        import rag_pdf_gradio_app as app

        process = mock.Mock()
        process.wait.side_effect = subprocess.TimeoutExpired("worker", 10)
        process.poll.return_value = None
        original_terminate = app.terminate_automatic_run_worker
        try:
            app.terminate_automatic_run_worker = lambda _root: True
            stopped = app.stop_automatic_run_worker_and_wait("C:/temp/app-run", process)
        finally:
            app.terminate_automatic_run_worker = original_terminate
    ...
    def test_grouped_root_ledger_is_discoverable_with_its_batch_configuration(self):
        import rag_pdf_gradio_app as app

        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir) / "app-run-current"
            root.mkdir()
            (root / ".automatic-batch-upload-config.json").write_text(
                json.dumps({
                    "api_url": "http://127.0.0.1:3009",
                    "provided_api_key": "batch-key",  # pragma: allowlist secret -- synthetic recovery fixture
                }),
                encoding="utf-8",
            )
            (root / "batch-embedding-ledger.json").write_text(
                json.dumps({"workspace_slug": "grouped-workspace", "batches": [
                    {"submission_state": "unresolved", "locations": ["custom-documents/grouped.json"]}
                ]}),
                encoding="utf-8",
            )
            groups = list(app._recovery_ledger_groups(root))

        self.assertEqual(len(groups), 1)
        self.assertEqual(groups[0]["workspace_slug"], "grouped-workspace")
        self.assertEqual(groups[0]["api_url"], "http://127.0.0.1:3009")
        self.assertNotIn("provided_api_key", groups[0])
        self.assertEqual(groups[0]["credential_source"], "not-retained")
    ...
    def test_terminal_run_status_reconciles_the_full_ui_after_a_lost_stream(self):
        ...
        self.assertNotIn("Compl:", updates[1])
        self.assertIn('aria-hidden="true"', updates[1])
        self.assertFalse(updates[2]["visible"])
        self.assertEqual(updates[3]["value"], "Completed with diagnostic notice")
        self.assertFalse(updates[3]["interactive"])
        self.assertNotIn("visible", updates[4])
    ...
    def test_download_folder_control_is_visually_grouped_with_downloads(self):
        ...
        self.assertIn('elem_id="batch-folder-file-selector"', source)
    ...
    def test_background_log_retention_keeps_recent_and_malformed_records(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            path.write_text(
                "\n".join([
                    ...
                    json.dumps({"recorded_at": app.datetime.now(app.timezone.utc).isoformat(), "state": "recent-aware"}),
                    json.dumps(["non-object-history-record"]),
        ...
        self.assertIn('"state": "recent-aware"', remaining)
        self.assertIn('["non-object-history-record"]', remaining)
        self.assertIn("malformed-for-manual-inspection", remaining)

    def test_timing_history_ignores_one_malformed_jsonl_line_without_losing_valid_evidence(self):
        import rag_pdf_gradio_app as app
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            path.write_text(
                "\n".join([
                    json.dumps({"run_key": "first", "actual_seconds": 10}),
                    "{partial-write",
                    json.dumps({"run_key": "second", "actual_seconds": 20}),
                    json.dumps(["non-object-record"]),
                ]) + "\n",
                encoding="utf-8",
            )
            rows = app._read_timing_jsonl(path)
    ...
    def test_per_run_timing_timeline_stays_valid_under_overlapping_callbacks(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
                ...
                writers = [
                    threading.Thread(
                        target=lambda index=index: (
                            start.wait(),
                            app.record_timing_model_event(root, "Embedding", {"batch": index}),
                        ),
                    )
            ...
            timeline = root / "timing-evidence-timeline.jsonl"
        ...
        self.assertEqual(sorted(row["batch"] for row in rows), list(range(12)))

    def test_ingestion_history_keeps_valid_runs_visible_when_one_jsonl_record_is_bad(self):
        import rag_pdf_gradio_app as app

        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "ingestion-history.jsonl"
            path.write_text(
                "\n".join([
                    json.dumps({
                        ...
                        "documents": [{"name": "valid.pdf", "runtime_validation": "pass"}, "invalid-document"],
                    }),
                    json.dumps(["non-object-record"]),
                    "{interrupted-json",
                ]) + "\n",
            ...
            original_path = app.INGESTION_HISTORY_PATH
            try:
                app.INGESTION_HISTORY_PATH = path
                rendered = app.ingestion_history_html("research")
            finally:
                app.INGESTION_HISTORY_PATH = original_path
    ...
    def test_version_and_system_theme_are_explicit_ui_contracts(self):
        ...
        # Theme colours belong to the shared CSS variables. Direct per-node
        # painting caused the former white/black hydration flash.
        self.assertNotIn('node.style.setProperty("background", themeControl.background, "important")', app.APP_JS)
        self.assertIn("#follow-windows-theme", app.APP_CSS)
    ...
    def test_workspace_model_configuration_is_provider_neutral(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                con.execute(
                    "create table workspaces(id integer primary key,name text,slug text,chatProvider text,chatModel text,topN integer,similarityThreshold real,vectorSearchMode text,chatMode text)"
                )
                con.execute(
                    "insert into workspaces values (1,'Deneen Vance','deneen-vance','generic-openai','qwen3-8-max',4,0.25,'default','query')"
                )
                con.commit()
            finally:
                con.close()

            configuration = pipeline.read_workspace_model_configuration(storage, "deneen-vance")

        self.assertEqual(configuration["status"], "configured")
        self.assertTrue(configuration["chat_model_configured"])
        self.assertEqual(configuration["chat_model"], "qwen3-8-max")
    ...
    def test_validation_document_cleanup_removes_only_exact_probe_locations(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            storage = Path(tmpdir)
            location = "custom-documents/raw-contract-probe-123.json"
            document = storage / "documents" / location
            document.parent.mkdir(parents=True)
            document.write_text("{}", encoding="utf-8")
            original_delete_json = pipeline.delete_json
            calls = []
            try:
                def fake_delete(url, api_key=None, timeout=30, body=None):
                    calls.append((url, api_key, timeout, body))
                    document.unlink()
                    return 200, '{"success":true}'

                pipeline.delete_json = fake_delete
                result = pipeline.cleanup_validation_document_records(
                    "http://127.0.0.1:3001",
                    "temporary-key",
                    [location],
                    storage_dir=storage,
                    workspace_slug="probe-workspace",
                )
            finally:
        ...
        self.assertEqual(result["confirmed_absent"], 1)
        self.assertEqual(calls[0][3], {"names": [location]})
        self.assertTrue(calls[0][0].endswith("/api/v1/system/remove-documents"))

    def test_validation_document_cleanup_rejects_non_probe_location_without_api_call(self):
        original_delete_json = pipeline.delete_json
        try:
            pipeline.delete_json = lambda *_args, **_kwargs: self.fail("unsafe cleanup reached API")
            result = pipeline.cleanup_validation_document_records(
                "http://127.0.0.1:3001",
                "temporary-key",
                ["custom-documents/user-document.json"],
                workspace_slug="probe-workspace",
            )
        finally:
            pipeline.delete_json = original_delete_json

        self.assertEqual(result["status"], "rejected_unmanaged_location")
    ...
    def test_run_temporary_workspace_validation_can_cleanup_after_success(self):
        ...
        try:
            ...
            pipeline.verify_anythingllm_post_upload = lambda *args, **kwargs: {
                "status": "pass", "matching_vector_rows": 1,
            }
    ...
    def test_temporary_validation_exact_vectors_are_not_failed_by_drawer_advisory(self):
        evidence = pipeline.normalize_temporary_validation_exact_evidence(
            {
                "status": "review",
                "classification": "desktop_drawer_layout_nested",
                "matching_vector_rows": 2,
        ...
        self.assertEqual(evidence["status"], "pass_with_missing_workspace_document_records")
        self.assertEqual(evidence["advisory_classification"], "desktop_drawer_layout_nested")

        partial = pipeline.normalize_temporary_validation_exact_evidence(
            {
                "status": "review",
                "matching_vector_rows": 1,
                "current_upload_vector_evidence_complete": False,
                "identity_set_complete": False,
                "chunk_survival_flag": "partial_or_missing",
            },
            2,
        )
        self.assertEqual(partial["status"], "review")
    ...
    def test_temporary_validation_does_not_delete_an_ambiguous_active_workspace(self):
        original_create = pipeline.create_validation_workspace
        original_upload = pipeline.maybe_upload_to_anythingllm
        original_delete = pipeline.delete_validation_workspace
        delete_calls = []
        try:
            pipeline.create_validation_workspace = lambda *args, **kwargs: {
                "status": "created", "workspace_slug": "ambiguous-test", "workspace_name": "Ambiguous Test"
            }
            pipeline.maybe_upload_to_anythingllm = lambda *args, **kwargs: {
                "status": "reconciliation_pending",
                "uploaded": 2,
                "embedded": 0,
                "locations": ["custom/a.json", "custom/b.json"],
                "errors": [{"error": "client response timed out"}],
            }
            pipeline.delete_validation_workspace = lambda *args, **kwargs: delete_calls.append(True)
            result = pipeline.run_temporary_workspace_validation(
                "http://127.0.0.1:3001", "api-key", PROJECT_ROOT, "abc123",
                [{"textContent": "x", "metadata": {"chunkSource": "segment://1"}}],
        ...
        self.assertEqual(result["status"], "upload_failed")
        self.assertEqual(result["retention_status"], "retained_for_ambiguous_mutation_reconciliation")
        self.assertEqual(result["cleanup_result"]["status"], "deferred_ambiguous_mutation")
        self.assertEqual(delete_calls, [])

    def test_temporary_validation_retries_review_only_evidence_until_exact_vectors(self):
        original_create = pipeline.create_validation_workspace
        original_upload = pipeline.maybe_upload_to_anythingllm
        original_post = pipeline.verify_anythingllm_post_upload
        ...
        try:
            pipeline.create_validation_workspace = lambda *args, **kwargs: {
                "status": "created", "workspace_slug": "exact-gate", "workspace_name": "Exact Gate"
            }

            def fake_upload(*_args, **kwargs):
                verification = kwargs["batch_verifier"]({
                    "start_index": 0, "end_index": 2, "locations": ["custom/a.json", "custom/b.json"],
                })
            ...
            verification_calls = [
                {"status": "verified_unavailable", "matching_vector_rows": 1},
                {"status": "pass", "matching_vector_rows": 2},
                {"status": "pass", "matching_vector_rows": 2},
            ]
            pipeline.verify_anythingllm_post_upload = lambda *args, **kwargs: verification_calls.pop(0)

            def fake_poll(inspector, **_kwargs):
                first = dict(inspector())
                observed_statuses.append(first["status"])
                if first["status"] == "partial_vector_coverage":
                    final = dict(inspector())
                else:
                    final = first
                return SimpleNamespace(
                    status="pass", final_evidence=final, attempts=2,
                    elapsed_seconds=0.1, observer_failures=[],
                )
        ...
        self.assertEqual(result["status"], "complete")
        self.assertIn("partial_vector_coverage", observed_statuses)
    ...
    def test_advanced_diagnostics_picker_uses_readable_pdf_validation(self):
        ...
        self.assertIn("not ready for diagnostics", updates[-1]["value"])
    ...
    def test_automatic_error_closes_a_live_run_before_the_observer_can_repaint(self):
        ...
        self.assertEqual(status["phase"], "Run stopped")
        self.assertEqual(status["completion_code"], "AUTO-TEST-FAIL")
        self.assertNotIn("AUTO-TEST-FAIL", status["details"])
        self.assertNotIn("AUTO-TEST-FAIL", outputs[0]["value"])
    ...
    def test_confirmed_run_reports_missing_or_invalid_state_without_silence(self):
        ...
        self.assertIn("run confirmation", result[0]["value"])
        self.assertNotIn("AUTO-CONFIRM-001", result[0]["value"])
        self.assertTrue(result[8]["visible"])
        self.assertNotIn("AUTO-CONFIRM-001", result[8]["value"])
    ...
    def test_confirmed_run_converts_unexpected_dispatch_error_into_visible_report(self):
        ...
        self.assertEqual(len(result), 9)
        self.assertIn("The confirmed run could not start", result[0]["value"])
        self.assertNotIn("AUTO-RUN-UNEXPECTED-001", result[0]["value"])
        self.assertTrue(result[8]["visible"])
    ...
    def test_confirmed_run_surfaces_handled_pipeline_error_outside_output_accordion(self):
        ...
        self.assertEqual(len(result), 9)
        self.assertTrue(result[8]["visible"])
        self.assertIn("Run stopped — processing", result[8]["value"])
        self.assertNotIn("AUTO-RUN-RESULT-001", result[8]["value"])
    ...
    def test_confirm_stream_acknowledges_before_the_long_pipeline_returns(self):
        ...
        self.assertEqual(preprocessing[9]["value"], "Processing…")
        ...
        self.assertEqual(started[9]["value"], "Processing…")
    ...
    def test_run_action_rows_keep_confirmation_controls_mounted(self):
        ...
        self.assertEqual(len(confirm_dependencies[0]["outputs"]), 16)
    ...
    def test_gradio_event_outputs_all_have_rendered_components(self):
        """A dangling output makes Gradio drop an otherwise valid stream.

        Buttons mounted inside a File component's header are usable event
        targets, but Gradio 6 does not expose them as independent rendered
        components. They therefore cannot be included in another event's
        output list.
        """
        import rag_pdf_gradio_app as app
        ...
        dangling = [
            (dependency.get("api_name"), output_id)
    ...
    def test_run_timer_estimate_refreshes_for_mode_and_upload_scope(self):
        ...
        local_model_seconds = int(re.search(r'data-model-expected-seconds="(\d+)"', local_timer).group(1))
        upload_model_seconds = int(re.search(r'data-model-expected-seconds="(\d+)"', upload_timer).group(1))
    ...
    def test_scan_selected_pdf_directory_keeps_only_pdf_candidates(self):
        ...
        self.assertIn("Successful. Selected folder:", status_html)
        self.assertIn("1 non-PDF files", status_html)
    ...
    def test_folder_picker_recurses_and_streams_a_visible_progress_state(self):
        ...
        self.assertNotIn("visible", selection_accordion)
        self.assertIn("Successful.", status["value"])
    ...
    def test_batch_folder_specific_selection_is_reversible(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # The browser has already removed its native × chip. Returning a
            # selector value here would allow an older response to restore a
            # file removed in a later click.
            self.assertEqual(result[2].get("__type__"), "update")
            self.assertNotIn("value", result[2])
            self.assertNotIn("visible", result[2])
            self.assertIn("PDFs Found (2)", result[3]["value"])
            self.assertNotIn("PDFs selected", result[3]["value"])

    def test_batch_folder_chip_removal_keeps_the_latest_browser_value(self):
        """Rapid native × removals must not receive an older selector value."""
        import rag_pdf_gradio_app as app

        config = app.demo.config
        component_names = {
            component["id"]: (component.get("props") or {}).get("elem_id")
            for component in config.get("components") or []
        }
        target = ("batch-folder-file-selector", "input")
        dependencies = [
            dependency
            for dependency in config.get("dependencies") or []
            if target in [
                (component_names.get(component_id), event_name)
                for component_id, event_name in dependency.get("targets") or []
            ]
        ]
        self.assertEqual(len(dependencies), 1)
        dependency = dependencies[0]
        self.assertEqual(dependency.get("trigger_mode"), "always_last")
        self.assertFalse(dependency.get("queue"))
        self.assertNotIn(
            "batch-folder-file-selector",
            [component_names.get(component_id) for component_id in dependency.get("outputs") or []],
        )
    ...
    def test_empty_batch_scan_keeps_the_folder_chooser_available(self):
        ...
        self.assertNotIn("visible", selection_panel)
        self.assertTrue(picker_area["visible"])
        self.assertIn("Selected folder", status["value"])
        self.assertIn("PDFs Found (0)", status["value"])
        self.assertIn("1 non-PDF files", status["value"])
        self.assertEqual(button["value"], "Select PDF Folder Here")
    ...
    def test_pdf_picker_native_inspection_cache_is_bounded_lru(self):
        import rag_pdf_gradio_app as app
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            app.PDF_PICKER_NATIVE_INSPECTION_CACHE.clear()
            app.PDF_PICKER_NATIVE_INSPECTION_CACHE_LIMIT = 2
            app.automatic_full_native_text_coverage = counted_coverage
            try:
                app.pdf_picker_native_inspection(pdf_paths[0])
                app.pdf_picker_native_inspection(pdf_paths[1])
                app.pdf_picker_native_inspection(pdf_paths[0])
                app.pdf_picker_native_inspection(pdf_paths[2])
                app.pdf_picker_native_inspection(pdf_paths[1])
            finally:
                app.automatic_full_native_text_coverage = original_coverage
    ...
    def test_new_ordinary_picker_pdf_merges_into_existing_folder_batch(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            direct_result = app.merge_uploaded_pdfs_into_direct_selection(
                [str(direct_pdf)], manifest,
            )

        self.assertEqual(len(result), 7)
        self.assertEqual(len(direct_result), 6)
    ...
    def test_folder_candidate_starts_a_fresh_confirm_state(self):
        ...
        # Selection preparation is browser-owned. It must not impersonate an
        # active run in the process-global status or Confirm can remain locked.
        self.assertEqual(app.LIVE_AUTOMATIC_RUN_STATUS, {})
    ...
    def test_batch_folder_status_rejects_fake_pdf_files(self):
        ...
        self.assertIn("Issue: PDF input", report)
        self.assertNotIn("AUTO-INPUT-003", report)
    ...
    def test_native_mutation_gate_requires_each_operation_capability(self):
        import rag_pdf_gradio_app as app
        ...
        with mock.patch.object(
            app,
            "characterize_anythingllm_compatibility",
            return_value={"desktop_version": "1.16.0", "capabilities": capabilities},
        ):
            accepted = app.automatic_native_mutation_compatibility_report(
                "http://127.0.0.1:3001",
                require_settings_write=True,
                require_workspace_create=True,
            )
        self.assertEqual(accepted["status"], "pass")

        capabilities["can_poll_post_upload_state"] = {"status": "blocked"}
        with mock.patch.object(
            app,
            "characterize_anythingllm_compatibility",
            return_value={"desktop_version": "future", "capabilities": capabilities},
        ):
            blocked = app.automatic_native_mutation_compatibility_report(
                "http://127.0.0.1:3001"
            )
        self.assertEqual(blocked["status"], "blocked")
        self.assertEqual(blocked["blocked_capabilities"], ["can_poll_post_upload_state"])

    def test_native_mutation_gate_can_require_workspace_creation(self):
        import rag_pdf_gradio_app as app

        capabilities = {
            "can_upload_native_metadata": {"status": "supported"},
            "can_poll_post_upload_state": {"status": "supported"},
            "can_create_workspace": {"status": "blocked"},
        }
        with mock.patch.object(
            app,
            "characterize_anythingllm_compatibility",
            ...
            report = app.automatic_native_mutation_compatibility_report(
                "http://127.0.0.1:3001",
                require_workspace_create=True,
            )
        self.assertEqual(report["status"], "blocked")
        self.assertEqual(report["blocked_capabilities"], ["can_create_workspace"])

    def test_native_mutation_gate_rejects_remote_endpoint_without_characterizing_local_desktop(self):
        import rag_pdf_gradio_app as app

        with mock.patch.object(
            app,
            "characterize_anythingllm_compatibility",
            side_effect=AssertionError("remote gate must not characterize local Desktop"),
        ):
            result = app.automatic_native_mutation_compatibility_report(
                "https://anythingllm.example.invalid/api"
            )
        self.assertEqual(result["status"], "blocked")
        self.assertEqual(result["reason"], "native_mutation_contract_requires_local_desktop")
    ...
    def test_run_automatic_strips_anythingllm_api_url_for_local_only_runs(self):
        ...
        original_capacity = app.automatic_batch_output_capacity_preflight
        captured = {}
        try:
            app.validate_pdf_inputs = lambda files: (["C:\\tmp\\dummy.pdf"], None)
            app.automatic_batch_output_capacity_preflight = lambda *_args, **_kwargs: {
                "status": "pass", "message": "test fixture capacity"
            }
            ...
            app.automatic_batch_output_capacity_preflight = original_capacity
    ...
    def test_run_automatic_reports_folder_without_pdfs(self):
        ...
        self.assertIn("Issue</div><div class=\"summary-value\">PDF input", summary_html)
        self.assertNotIn("AUTO-INPUT-003", summary_html)
    ...
    def test_run_automatic_degrades_unavailable_simulation_and_still_prepares(self):
        ...
        try:
            ...
            app.automatic_native_mutation_compatibility_report = lambda *_args, **_kwargs: {
                "status": "pass", "blocked_capabilities": [], "characterization": {}
            }
        ...
        self.assertIn("Run stopped", native_result[0]["value"])
    ...
    def test_run_automatic_legacy_probe_scope_keeps_the_full_ledger_limit(self):
        ...
        try:
            ...
            def fake_prepare(pdf_path, out_dir, args):
                captured["prepare_and_upload"] = args.prepare_and_upload
                captured["defer_lean_retention"] = args.defer_lean_retention
        ...
        # Automatic upload mode stages this worker locally; the one shared
        # Desktop queue is submitted only after every selected PDF finishes.
        self.assertFalse(captured["prepare_and_upload"])
        self.assertTrue(captured["defer_lean_retention"])
        ...
        self.assertIn("Run stopped", result[0]["value"])
    ...
    def test_run_automatic_upload_mode_fails_fast_when_runtime_api_is_down(self):
        ...
        self.assertIn("AnythingLLM upload", result[0]["value"])
        self.assertNotIn("AUTO-UPLOAD-001", result[0]["value"])
        self.assertIn("AnythingLLM runtime API is not reachable", result[0]["value"])

    def test_run_automatic_blocks_unknown_mutation_contract_before_preparation(self):
        import rag_pdf_gradio_app as app
        ...
        readiness = {
            ...
            "workspace_slug_found": True,
            "workspace_slug_message": "workspace exists",
            "workspace_api_found": True,
            "workspace_api_message": "workspace exists",
            "runtime_api_url": "http://127.0.0.1:3001",
            "runtime_api_reachable": True,
            "runtime_api_status": "reachable",
        ...
        with (
            mock.patch.object(app, "validate_pdf_inputs", return_value=(["C:\\tmp\\dummy.pdf"], None)),
            mock.patch.object(app, "local_workspace_slug_exists", return_value=(True, "workspace exists")),
            mock.patch.object(app, "native_upload_readiness_report", return_value=readiness),
            mock.patch.object(app, "automatic_batch_output_capacity_preflight", return_value={"status": "pass", "message": "ok"}),
            mock.patch.object(
                app,
                "automatic_native_mutation_compatibility_report",
                return_value={
                    "status": "blocked",
                    "reason": "required_native_mutation_capability_not_qualified",
                    "blocked_capabilities": ["can_upload_native_metadata"],
                    "characterization": {"desktop_version": "future"},
                },
            ),
            mock.patch.object(app, "prepare_pdf", side_effect=AssertionError("preparation must not start")),
        ):
            result = app.run_automatic(
                ["C:\\tmp\\dummy.pdf"], [], "", "", "", True,
                app.MODE_NATIVE_UPLOAD_LABEL, str(PROJECT_ROOT / "tmp-output"),
                "http://127.0.0.1:3001", "", "workspace-a",
                app.NATIVE_UPLOAD_SCOPE_ALL_LABEL, "", "Strict metadata only",
                False, "", app.SIMULATION_SKIP_LABEL, "", app.DEFAULT_OLLAMA_URL,
                "Focused (up to 300 chunks)", False, False, False, "Automatic",
                0, 0, 750, 0, app.SEGMENT_PAGE_LIMIT_LABEL, "", "", "fast",
                True, True, 512, 75, False, True, False,
            )

        self.assertIn("AnythingLLM compatibility", result[0]["value"])
        self.assertNotIn("AUTO-COMPATIBILITY-001", result[0]["value"])
        self.assertIn("can_upload_native_metadata", result[0]["value"])
    ...
    def test_run_automatic_upload_mode_does_not_block_completed_upload_on_needs_review(self):
        ...
        original_embedder_probe = app.verify_anythingllm_runtime_embedder
        original_capacity = app.automatic_batch_output_capacity_preflight
        original_checkpoint_write = app.write_prepared_batch_checkpoint
        original_checkpoint_verify = app.verify_prepared_batch_checkpoint
        original_batch_upload = app.upload_prepared_automatic_batch
        original_terminal_audit = app.terminal_integrity_audit
        original_compatibility = app.automatic_native_mutation_compatibility_report
        try:
            app.validate_pdf_inputs = lambda files: (["C:\\tmp\\dummy.pdf"], None)
            app.automatic_batch_output_capacity_preflight = lambda *_args, **_kwargs: {
            ...
            app.write_prepared_batch_checkpoint = lambda run_root, *_args, **_kwargs: Path(run_root) / "prepared-batch-recovery-manifest.json"
            app.verify_prepared_batch_checkpoint = lambda *_args, **_kwargs: {
                "status": "verified", "reusable": True, "reason": "test fixture"
            }
            app.upload_prepared_automatic_batch = lambda *_args, **_kwargs: {
                "status": "complete", "uploaded": 2, "embedded": 2, "errors": [], "document_results": {}
            }
            app.terminal_integrity_audit = lambda _root, completion, **_kwargs: (completion, {})
            app.automatic_native_mutation_compatibility_report = lambda *_args, **_kwargs: {
                "status": "pass", "blocked_capabilities": [], "characterization": {}
            }
            app.local_workspace_slug_exists = lambda slug: (True, "workspace exists")
            ...
            def fake_legacy_summary(_run):
                ...
                return {
                    ...
                    "workspace_model_configuration_status": "configured",
                    "post_upload_verification_status": "pass",
            ...
            app.verify_anythingllm_runtime_embedder = original_embedder_probe
            app.automatic_batch_output_capacity_preflight = original_capacity
            app.write_prepared_batch_checkpoint = original_checkpoint_write
            app.verify_prepared_batch_checkpoint = original_checkpoint_verify
            app.upload_prepared_automatic_batch = original_batch_upload
            app.terminal_integrity_audit = original_terminal_audit
            app.automatic_native_mutation_compatibility_report = original_compatibility
    ...
    def test_run_summary_html_marks_completed_upload_with_review_flags_not_error(self):
        ...
        self.assertIn("Completed with diagnostic flags", rendered)
    ...
    def test_build_run_diagnostics_does_not_turn_workspace_configuration_into_a_gate(self):
    ...
    def test_build_run_diagnostics_renames_missing_outline_warning(self):
        diagnostics = pipeline.build_run_diagnostics(
            ...
            storage_report={"status": "complete"},
            upload_report={"status": "skipped_prepare_only"},
            workspace_configuration={"status": "not_checked", "message": ""},
            post_upload_report={"status": "not_checked"},
        ...
        try:
            ...
            def fake_legacy_summary(_run):
                ...
                return {
                    ...
                    "workspace_model_configuration_status": "not_checked",
                    "post_upload_verification_status": "not_checked",
    ...
    def test_detected_metadata_preview_includes_text_layer_profile_rows(self):
        ...
        original_infer_author = app.infer_author_from_initial_pdf_pages
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                app.infer_author_from_initial_pdf_pages = original_infer_author
    ...
    def test_automatic_run_json_writer_remains_valid_under_concurrent_status_updates(self):
        import rag_pdf_gradio_app as app

        with tempfile.TemporaryDirectory() as temp_dir:
            target = Path(temp_dir) / "run-progress.json"
            start = threading.Event()
            writers = [
                threading.Thread(
                    target=lambda index=index: (
                        start.wait(), app._write_automatic_run_json(target, {"writer": index})
                    ),
                )
                for index in range(12)
            ]
            ...
            self.assertTrue(all(not writer.is_alive() for writer in writers))
            final_record = json.loads(target.read_text(encoding="utf-8"))
            self.assertIn(final_record["writer"], range(12))
            self.assertEqual(list(Path(temp_dir).glob(".run-progress.json.*.tmp")), [])

    def test_overlapping_live_status_updates_do_not_lose_the_higher_confirmed_checkpoint(self):
        import rag_pdf_gradio_app as app

        with tempfile.TemporaryDirectory() as temp_dir:
            original_status = app.LIVE_AUTOMATIC_RUN_STATUS
            original_write = app.atomic_write_text
            first_write_entered = threading.Event()
            release_first_write = threading.Event()
            ...
            def delayed_first_write(*args, **kwargs):
                ...
                if is_first:
                    ...
                    if not release_first_write.wait(timeout=3):
                        raise TimeoutError("test did not release first status write")
            ...
            first = threading.Thread(target=update, args=(0.8,))
            second = threading.Thread(target=update, args=(0.1,))
            try:
                app.LIVE_AUTOMATIC_RUN_STATUS = {}
                app.atomic_write_text = delayed_first_write
                first.start()
                ...
                final_status = dict(app.LIVE_AUTOMATIC_RUN_STATUS)
            finally:
                app.atomic_write_text = original_write
                app.LIVE_AUTOMATIC_RUN_STATUS = original_status

        self.assertFalse(first.is_alive())
        self.assertFalse(second.is_alive())
        self.assertEqual(errors, [])
        self.assertEqual(final_status["confirmed_fraction"], 0.8)
    ...
    def test_custom_range_control_is_editable_only_for_one_pdf_and_confirmation_keeps_reason(self):
        ...
        try:
            ...
            app.validated_automatic_run_settings = original_validate
        self.assertIn("Issue</div><div class=\"summary-value\">run confirmation", result[0]["value"])
        self.assertNotIn("AUTO-NATIVE-RANGE-001", result[0]["value"])
    ...
    def test_custom_range_batch_is_rejected_before_ocr_preflight(self):
        ...
        self.assertIn("Issue: processing", report)
        self.assertNotIn("AUTO-NATIVE-RANGE-001", report)
    ...
    def test_custom_range_passage_mode_is_rejected_before_ocr_preflight(self):
        ...
        self.assertEqual(warnings, [])
        self.assertIn("Issue: processing", report)
        self.assertNotIn("AUTO-NATIVE-RANGE-002", report)
    ...
    def test_no_segmentation_target_policy_and_pipeline_mapping_are_explicit(self):
        ...
        # Automatic, Advanced diagnostics, and the separate future-defaults
        # editor intentionally expose the same segmentation policy. The
        # defaults editor is not a third live-run implementation.
        self.assertEqual(len(segment_components), 3)
    ...
    def test_runtime_native_validation_checks_citation_for_configured_model(self):
        payload = {
        ...
        original_gate = pipeline.read_workspace_model_configuration
        original_post = pipeline.post_json_captured
        try:
            pipeline.read_workspace_model_configuration = lambda *args, **kwargs: {
                "status": "pass",
            ...
            pipeline.read_workspace_model_configuration = original_gate
    ...
    def test_runtime_native_validation_separates_chat_timeout_from_citation_failure(self):
        ...
        try:
            ...
            def fake_post(url, body, api_key=None, timeout_label="request", timeout_seconds=120):
                ...
                return {"http_status": 0, "data": {}, "error": "AnythingLLM chat failed: timed out"}
    ...
    def test_region_aware_native_layout_reports_scan_and_targeted_ocr_activity(self):
        with tempfile.TemporaryDirectory() as tmp:
            pdf_path = Path(tmp) / "layout-progress.pdf"
            document = pipeline.fitz.open()
            page = document.new_page(width=612, height=792)
            page.insert_text((55, 100), "Ordinary body prose remains visible.", fontsize=10)
            document.save(pdf_path)
            document.close()
            events = []
            with pipeline.fitz.open(pdf_path) as source:
            ...
            with mock.patch.object(
                ...
                pipeline.apply_region_aware_native_layout(
                    pdf_path,
                    pages,
                    progress_callback=lambda completed, total, activity: events.append(
                        (completed, total, activity)
                    ),
                )
        activities = [activity for _completed, _total, activity in events]
        self.assertIn("native_layout_scan", activities)
        self.assertIn("targeted_ocr_started", activities)
    ...
    def test_env_file_reader_decodes_json_quoted_values(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            env_path = Path(temp_dir) / ".env"
            expected = 'model with "quotes", apostrophe\'s, and \\slashes'
            env_path.write_text(
                f"EMBEDDING_MODEL_PREF={json.dumps(expected)}\n",
                encoding="utf-8",
            )

            values = pipeline.read_env_file_values(env_path)

        self.assertEqual(values["EMBEDDING_MODEL_PREF"], expected)
    ...
    def test_anythingllm_env_writer_rejects_line_injection_and_keeps_prior_settings(self):
        with tempfile.TemporaryDirectory() as storage_dir:
            storage = Path(storage_dir)
            env_path = storage / ".env"
            original = "EMBEDDING_ENGINE='ollama'\n"
            env_path.write_text(original, encoding="utf-8")

            with self.assertRaisesRegex(ValueError, "single-line"):
                pipeline.write_anythingllm_env_value(
                    storage,
                    "EMBEDDING_ENGINE",
    ...
    def test_related_embedder_env_update_does_not_partially_write_if_atomic_replace_fails(self):
        with tempfile.TemporaryDirectory() as storage_dir:
            storage = Path(storage_dir)
            env_path = storage / ".env"
    ...
    def test_overlapping_anythingllm_env_updates_preserve_both_settings(self):
        with tempfile.TemporaryDirectory() as storage_dir:
            storage = Path(storage_dir)
            env_path = storage / ".env"
            ...
            def delayed_first_write(*args, **kwargs):
                ...
                if is_first:
                    first_write_entered.set()
                    if not release_first_write.wait(timeout=3):
                        raise TimeoutError("test did not release first settings write")
                return original_write(*args, **kwargs)

            def update(key, value):
                try:
                    pipeline.write_anythingllm_env_value(storage, key, value)
                except Exception as exc:  # pragma: no cover - asserted below
                    errors.append(exc)
            ...
            self.assertEqual(values["EMBEDDING_ENGINE"], "openai")
            self.assertEqual(values["EMBEDDING_MODEL_PREF"], "text-embedding-3-small")
    ...
    def test_author_confidence_rejects_document_roles_without_banning_words(self):
        self.assertFalse(pipeline.looks_like_person_name("Credits (ECTS)"))
        self.assertFalse(pipeline.looks_like_person_name("EIGHTH EDITION"))
        self.assertTrue(pipeline.looks_like_person_name("Maya Edition"))
        self.assertTrue(pipeline.looks_like_person_name("Maya Bold"))

    def test_pdf_author_metadata_rejects_font_style_values_only(self):
        self.assertEqual(pipeline.normalize_metadata_author("Dorèl Extra Bold"), "")
        self.assertEqual(pipeline.normalize_metadata_author("Sample Semi Bold Italic"), "")
        self.assertEqual(pipeline.normalize_metadata_author("Maya Bold"), "Maya Bold")

    def test_title_adjacent_author_inference_abstains_from_course_field(self):
        report = pipeline.infer_author_from_text_samples(
            [{
                "page": 1,
                "text": "Example Course\nCredits (ECTS)\n5\nCourse description follows.",
            }],
            title_hint="Example Course",
        )
        self.assertNotEqual(report.get("author"), "Credits (ECTS)")

    def test_stacked_title_page_does_not_promote_edition_statement(self):
        report = pipeline.infer_author_from_text_samples(
            [{
                "page": 1,
                "text": "FILM ART\nEIGHTH EDITION\nMcGraw-Hill\n",
        ...
        self.assertNotEqual(report.get("author"), "EIGHTH EDITION")
    ...
    def test_retry_picker_helpers_keep_existing_valid_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            selected, manifest, picker_update, _boundary_update, _status = app.reuse_selected_pdf_batch(
    ...
    def test_upload_plan_rows_to_expected_payloads_models_multipart_storage_identity(self):
        rows = [
            {
        ...
        self.assertEqual(payloads[0]["metadata"]["title"], "sample-p0009-s00001")
        self.assertEqual(payloads[0]["metadata"]["chunkSource"], "segment://sample-1")
        self.assertEqual(payloads[0]["metadata"]["description"], "PDF page 9; segment one.")
    ...
    def test_deferred_source_attachment_returns_locations_to_its_queue_group(self):
        """Deferred source grouping must not misclassify its own hand-off as an error."""
        with tempfile.TemporaryDirectory() as temp_dir:
            text_file = Path(temp_dir) / "prepared.txt"
            text_file.write_text("prepared page text", encoding="utf-8")
            row = {
                "filename": "prepared.txt",
                "text_file": str(text_file),
                "chunkSource": "page-parent://test-p0001",
                "docSource": "local-pdf://sha256/" + "a" * 64,
                "_automatic_source_path": "C:/sources/test.pdf",
                "_validated_prepared_file_sha256": pipeline.hashlib.sha256(
            ...
            with (
                ...
                mock.patch.object(
                    pipeline,
                    "snapshot_staged_document_locations",
                    return_value=set(),
                ),
                mock.patch.object(
                    pipeline,
                ...
                mock.patch.object(
                    pipeline,
                    "extract_document_location",
                    return_value="custom-documents/prepared.json",
                ),
            ):
                result = pipeline.maybe_upload_segment_files(
                    "http://anythingllm",
                    "key",
                    [row],
        ...
        self.assertEqual(result["status"], "attached_pending_queue")
        self.assertEqual(result["locations"], ["custom-documents/prepared.json"])
        self.assertEqual(result["errors"], [])

    def test_source_queue_group_plan_preserves_a_large_pdf_as_one_group(self):
        groups = [
            {"source_key": "A", "rows": [{}, {}]},
            {"source_key": "B", "rows": [{}]},
            {"source_key": "C", "rows": [{} for _ in range(600)]},
            {"source_key": "D", "rows": [{}]},
        ]

        planned = pipeline.plan_source_queue_groups(
            groups, max_sources=4, max_records=10,
        )

        self.assertEqual(
            [[item["source_key"] for item in group] for group in planned],
            [["A", "B"], ["C"], ["D"]],
        )

    def test_source_transactions_submit_one_bounded_queue_group_after_source_local_attachment(self):
        rows = [
            {
                "_automatic_source_path": "C:/sources/A.pdf",
                "chunkSource": "page-parent://a-1",
                "docSource": "local-pdf://sha256/" + "a" * 64,
            },
            {
                "_automatic_source_path": "C:/sources/B.pdf",
                "chunkSource": "page-parent://b-1",
                "docSource": "local-pdf://sha256/" + "b" * 64,
            },
            {
                "_automatic_source_path": "C:/sources/C.pdf",
                "chunkSource": "page-parent://c-1",
                "docSource": "local-pdf://sha256/" + "c" * 64,
            },
        ]
        calls = []
        ...
        def fake_segment(_url, _key, source_rows, **kwargs):
            ...
            location = "custom/" + Path(source_path).stem + ".json"
            return {
        ...
        def fake_queue(_url, _key, _workspace, locations, **kwargs):
            calls.append((list(locations), kwargs))
            self.assertTrue(kwargs["_source_window_execution"])
            verifier = kwargs["batch_verifier"]
            verifier({"locations": list(locations)})
            return {
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with (
                ...
                result = pipeline.maybe_upload_segment_files_source_transactions(
                    ...
                    embedding_ledger_path=Path(temp_dir) / "batch-embedding-ledger.json",
                    batch_verifier=lambda report: self.assertEqual(
                        report["source_paths"],
                        ["C:/sources/A.pdf", "C:/sources/B.pdf", "C:/sources/C.pdf"],
                    ) or {},
                )
    ...
    def test_source_local_rejection_does_not_discard_other_sources_in_its_queue_group(self):
        rows = [
            {"_automatic_source_path": "C:/sources/A.pdf", "chunkSource": "a", "docSource": "local-pdf://sha256/" + "a" * 64},
            {"_automatic_source_path": "C:/sources/B.pdf", "chunkSource": "b", "docSource": "local-pdf://sha256/" + "b" * 64},
            {"_automatic_source_path": "C:/sources/C.pdf", "chunkSource": "c", "docSource": "local-pdf://sha256/" + "c" * 64},
        ]
        queued = []

        def fake_segment(_url, _key, source_rows, **_kwargs):
            source_path = source_rows[0]["_automatic_source_path"]
            if source_path.endswith("B.pdf"):
                return {
                    "status": "error",
                    "uploaded": 0,
                    "embedded": 0,
            ...
            location = "custom/" + Path(source_path).stem + ".json"
            return {
                "status": "attached_pending_queue", "uploaded": 1, "embedded": 0,
                "locations": [location], "reused_cached_locations": [],
                "attachment_results": [{"location": location, "source_path": source_path, "filename": Path(source_path).name}],
                "embedding_update": {}, "errors": [],
            }

        def fake_queue(_url, _key, _workspace, locations, **_kwargs):
            queued.append(list(locations))
            return {
                "accepted": len(locations), "requested": len(locations),
                "queue_records": len(locations), "planned_locations": list(locations),
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with (
                ...
                result = pipeline.maybe_upload_segment_files_source_transactions(
                    "http://anythingllm", "key", rows,
                    workspace_slug="queue-workspace", storage_dir=temp_dir,
                )

        self.assertEqual(queued, [["custom/A.json", "custom/C.json"]])
        states = {row["source_filename"]: row["state"] for row in result["source_transactions"]}
        self.assertEqual(states["A.pdf"], "exact_vectors_proven")
        self.assertEqual(states["B.pdf"], "source_rejected_without_remote_mutation")
        self.assertEqual(states["C.pdf"], "exact_vectors_proven")

    def test_ambiguous_queue_group_holds_later_groups_without_replaying_the_group(self):
        rows = [
            {"_automatic_source_path": f"C:/sources/{name}.pdf", "chunkSource": name, "docSource": "local-pdf://sha256/" + name.casefold() * 64}
        ...
        def fake_queue(_url, _key, _workspace, locations, **_kwargs):
            queued.append(list(locations))
            return {
                "accepted": 0, "requested": len(locations), "queue_records": len(locations),
                "planned_locations": list(locations),
                "batches": [{"submission_state": "unresolved", "searchability_proven": False}],
                "runtime_events": [],
                "errors": [{"category": "ambiguous_external_mutation", "error": "receipt unknown"}],
            }
        ...
        self.assertEqual(len(queued[0]), 4)
        self.assertEqual(result["stopped_after_source_transaction"], 1)
        self.assertEqual(len(result["source_transactions"]), 4)
        self.assertTrue(all(
            row["state"] == "ambiguous_external_mutation_held"
            for row in result["source_transactions"]
        ))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            pipeline._write_embedding_batch_ledger(
                ...
                "queue-workspace",
                {
                    "requested": 2,
                    "accepted": 0,
                    "planned_locations": ["custom/a.json", "custom/b.json"],
                    "batches": [{
                        "batch": 1,
                        "start_index": 0,
                        "submission_state": "rejected",
                        "locations": ["custom/a.json"],
                    }],
                    "runtime_events": [],
                    "location_sources": [
                        {"location": "custom/a.json", "source_path": "C:/sources/A.pdf", "filename": "A.pdf"},
                        {"location": "custom/b.json", "source_path": "C:/sources/B.pdf", "filename": "B.pdf"},
                    ],
                },
            )
            ledger = json.loads(ledger_path.read_text(encoding="utf-8"))
    ...
    def test_terminal_integrity_failure_overrides_nominal_success_without_retry(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                mock.patch.object(app, "write_failure_bundle", return_value=root / "reliability-failures" / "audit-1"),
            ):
                completion, retained = app.terminal_integrity_audit(
                    root, {"state": "successful", "message": "ready"}, native_run=True,
                )
    ...
    def test_terminal_integrity_observer_failure_does_not_erase_a_pipeline_failure(self):
        ...
        original = {"state": "failed", "code": "AUTO-UPLOAD-001", "message": "upload failed"}
        with tempfile.TemporaryDirectory() as temp_dir:
            with mock.patch.object(app, "audit_run_directory", side_effect=OSError("locked")):
                completion, retained = app.terminal_integrity_audit(
    ...
    def test_terminal_integrity_bundle_failure_does_not_soften_proven_contradiction(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with (
                mock.patch.object(app, "audit_run_directory", return_value=audit),
                mock.patch.object(app, "write_failure_bundle", side_effect=OSError("disk full")),
            ):
                completion, retained = app.terminal_integrity_audit(
    ...
    def test_embedding_submission_receipt_deadline_override_starts_reconciliation_early(self):
        ...
        self.assertEqual(
            result["batches"][0]["receipt_state"],
            "deadline_elapsed_vectors_confirmed",
        )
        self.assertEqual(
            result["batches"][0]["resolved_transport_event"]["message"],
            "The client response deadline elapsed while AnythingLLM continued processing; exact-vector observation is in progress.",
        )
        self.assertEqual(
            result["batches"][0]["resolved_transport_event"]["raw_transport_error"],
            "AnythingLLM request failed: timed out",
        )
    ...
    def test_runtime_retrieval_accepts_expected_source_within_top_n(self):
        ...
        try:
            pipeline.read_workspace_model_configuration = lambda *_args, **_kwargs: {"status": "configured"}
    ...
    def test_embedding_scheduler_serializes_after_exact_vector_evidence(self):
        ...
        try:
            ...
            result = pipeline.update_workspace_embeddings_batched(
                ...
                batch_verifier=lambda report: {
                    "status": "pass",
                    "matching_vector_rows": len(report.get("locations") or []),
                },
            )
        finally:
            pipeline.post_json = original_post
    ...
    def test_embedding_cancellation_during_rate_limit_wait_never_retries(self):
        original_post = pipeline.post_json
        locations = ["custom-documents/segment-1.json", "custom-documents/segment-2.json"]
        calls = []
        with tempfile.TemporaryDirectory() as temp_dir:
            ledger = Path(temp_dir) / "embedding-batch-ledger.json"
            try:
                pipeline.post_json = lambda *_args, **_kwargs: (calls.append(1) or 429, "Too Many Requests")
                result = pipeline.update_workspace_embeddings_batched(
                    "http://anythingllm",
                    "key",
                    "safe-workspace",
                    locations,
                    ledger_path=ledger,
                    cancel_callback=lambda: bool(calls),
                    verification_mode="none",
                )
            finally:
                pipeline.post_json = original_post
        ...
        self.assertEqual(calls, [1])
        self.assertEqual(result["accepted"], 0)
        self.assertEqual(result["batches"][0]["submission_state"], "cancelled_before_retry")
        self.assertEqual(result["errors"][0]["endpoint"], "operator-cancellation")
        self.assertEqual(recovery["recovery"]["remaining_locations"], locations)
    ...
    def test_cleanup_warning_promotes_final_run_to_needs_review(self):
        ...
        self.assertIn("not confirmed", upload_report["warnings"][0]["warning"].casefold())
    ...
    def test_resume_prefers_managed_local_service_key(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            documents = root / "documents"
            location = "custom/segment-6.json"
            prepared = documents / location
            prepared.parent.mkdir(parents=True)
            prepared.write_text(json.dumps({"docSource": "local-pdf://sha256/" + "a" * 64}), encoding="utf-8")
            manifest_path = root / "resume-embedding-manifest.json"
            manifest = {
                "workspace_slug": "safe-workspace",
                "batch_size": 5,
            ...
            calls = []
            with (
                mock.patch.object(app, "persist_reconciled_resume_manifest", return_value=([location], {"reconciled_locations": 0})),
                mock.patch.object(app, "default_anythingllm_documents_dir", return_value=documents),
                mock.patch.object(app, "default_anythingllm_storage_dir", return_value=root / "storage"),
                mock.patch.object(app, "resolve_anythingllm_api_key", return_value=("managed-secret", "managed_local_service_key")),
                mock.patch.object(app, "create_temporary_desktop_api_key", side_effect=AssertionError("temporary key should not be created")),
                mock.patch.object(app, "observe_workspace_embedding_queue_activity", return_value={"status": "idle"}),
                mock.patch.object(app, "acquire_automatic_anythingllm_mutation_lease", return_value={"acquired": True}),
                mock.patch.object(app, "release_automatic_anythingllm_mutation_lease"),
                mock.patch.object(app, "is_lancedb_safe_namespace", return_value=True),
                mock.patch.object(
                    ...
                    side_effect=lambda api_url, key, slug, locations, **_kwargs: (
                        calls.append((api_url, key, slug, locations))
                        or {"accepted": len(locations), "errors": []}
                    ),
                ),
            ):
                result = app.submit_embedding_resume_manifest(
    ...
    def test_six_file_uploads_share_one_desktop_embedding_submission(self):
        """Six staged source files must become one Desktop queue payload."""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            upload_rows = []
            for index in range(1, 7):
                source = tmp_path / f"batch-source-{index}.txt"
                source.write_text(f"batch source {index}", encoding="utf-8")
                upload_rows.append(
                    {
                        "filename": source.name,
                        "title": f"Batch source {index}",
                        "docSource": f"local-pdf://sha256/source-{index}",
                        "chunkSource": f"batch://source-{index}",
            ...
            embedding_calls = []
            try:
                def fake_multipart(_url, fields, file_field_name, file_path, **_kwargs):
                    filename = Path(file_path).name
                    multipart_calls.append(filename)
                    return 200, json.dumps({"documents": [{"location": f"custom-documents/{filename}.json"}]})
                ...
                report = pipeline.maybe_upload_to_anythingllm(
                    ...
                    "key",
                    [],
                    workspace_slug="six-file-workspace",
                    upload_transport="file_upload",
                    upload_plan_rows=upload_rows,
                    batch_verifier=lambda batch: {
                        "status": "pass",
                        "matching_vector_rows": len(batch["locations"]),
                    },
                )
            finally:
    ...
    def test_later_ambiguous_attachment_preserves_prior_source_and_holds_following_sources(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows = [
                {
                    "filename": path.name,
                    "title": path.stem,
                    "docSource": f"local-pdf://sha256/{path.stem}",
                    "chunkSource": f"page-parent://{path.stem}",
                    "text_file": str(path),
                    "_automatic_source_path": f"{path.stem}.pdf",
                }
            ...
            try:
                def fake_multipart(*_args, **kwargs):
                    ...
                    if len(multipart_calls) == 1:
                        return 200, json.dumps({"documents": [{"location": "custom-documents/first.json"}]})
                    return 500, "attachment failed"
        ...
        # The first source was safely submitted before the later ambiguous
        # attachment held the group, but the mocked 2xx does not prove vectors.
        self.assertEqual(report["source_transactions"][0]["state"], "ambiguous_external_mutation_held")
    ...
    def test_explicit_source_attachment_rejection_releases_the_next_pdf(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "adds": [
                        "custom-documents/first.json",
                        "custom-documents/third.json",
                    ],
                    "deletes": [],
                },
            ],
        ...
        # The explicit rejection released the third source into the bounded
        # queue group. A bare mocked 2xx still cannot be called vector proof.
        self.assertEqual(report["source_transactions"][2]["state"], "ambiguous_external_mutation_held")
        self.assertEqual(report["status"], "reconciliation_pending")
        self.assertTrue(transaction_ledger["journal_finalized"])
        self.assertEqual(len(transaction_ledger["transactions"]), 3)
        # Group-level queue evidence adds durable state transitions; do not
        # make the contract depend on an incidental journal-event count.
        self.assertGreaterEqual(len(transaction_events), 9)
    ...
    def test_missing_attachment_location_recovery_does_not_claim_vectors_from_http_acceptance(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
        ...
        # The bounded queue route must retain recovery rather than reporting a
        ...
        self.assertTrue(all(
            row["state"] == "ambiguous_external_mutation_held"
            for row in report["source_transactions"]
        ))
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                def fake_post_multipart(
                    url, fields, file_field_name, file_path, api_key=None,
                    timeout=120, file_bytes=None,
                ):
    ...
    def test_batch_output_directories_do_not_collide_for_same_named_pdfs(self):
        """A batch must retain both sources even when Windows paths share a basename."""
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            first = root / "first" / "article.pdf"
            second = root / "second" / "article.pdf"
            first.parent.mkdir()
            second.parent.mkdir()
            for path, text in ((first, "first source"), (second, "second source")):
                document = fitz.open()
                document.new_page().insert_text((72, 72), text)
                document.save(path)
                document.close()

            destinations = pipeline.compatible_output_document_directories(
                root / "run", [first, second]
            )

        self.assertEqual(len(destinations), 2)
        self.assertNotEqual(destinations[str(first)], destinations[str(second)])
        self.assertIn("article", destinations[str(first)].name)
        self.assertIn("article", destinations[str(second)].name)

    def test_automatic_grouped_upload_submits_all_staged_pdfs_once(self):
        """The Automatic runner must not create one Desktop queue call per PDF."""
        import rag_pdf_gradio_app as app
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            summaries = []
            for index in range(1, 7):
                text_file = root / f"source-{index}.txt"
                plan_file = root / f"source-{index}-upload-plan.csv"
                text_file.write_text(f"Source {index} text", encoding="utf-8")
                plan_file.write_text(
                    "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                    f"source-{index}.txt,Source {index},,PDF {index},local-pdf://sha256/hash-{index},"
                    f"hash-{index}-p001,{text_file}\n",
                    encoding="utf-8",
                )
                summaries.append(
                    {
                        "pdf": str(root / f"source-{index}.pdf"),
            ...
            def fake_upload(*_args, **kwargs):
                ...
                upload_rows = kwargs["upload_plan_rows"]
                ...
                return {
                    ...
                    "attachment_results": [
                        {
                            ...
                            "location": f"custom-documents/{row['filename']}.json",
                            "status": "attached",
                            "error": "",
                        }
                    ...
                    "embedding_update": {
                        ...
                        "batches": [{
                            "submission_state": "accepted",
                            "searchability_proven": True,
                            "locations": [f"custom-documents/{row['filename']}.json" for row in upload_rows],
                            "verification": {
                                "observed_chunk_sources": [row["chunkSource"] for row in upload_rows],
                            },
                        }],
                    },
                }
            ...
            with mock.patch.object(app, "maybe_upload_to_anythingllm", side_effect=fake_upload), mock.patch.object(
                ...
                report = app.upload_prepared_automatic_batch(
                    ...
                    workspace_slug="batch-workspace",
                    run_root=root / "run",
                )

        self.assertEqual(len(captured), 1)
        self.assertEqual(len(captured[0]["upload_plan_rows"]), 6)
        self.assertEqual(report["status"], "complete")
    ...
    def test_grouped_upload_disambiguates_identical_segment_filenames_before_submission(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for source in ("first", "second"):
                ...
                plan_file.write_text(
                    "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                    f"shared-p001-s00001.txt,Shared,,,local-pdf://sha256/{source},{source}-p001,{text_file}\n",
                    encoding="utf-8",
                )
                summaries.append(
                    {
                        ...
                        "batches": [
                            {
                                ...
                                "verification": {
                                    "observed_chunk_sources": [row["chunkSource"] for row in rows],
                                },
                            }
                        ],
                    },
            ...
            with mock.patch.object(app, "maybe_upload_to_anythingllm", side_effect=fake_upload), mock.patch.object(
                ...
                report = app.upload_prepared_automatic_batch(
                    summaries,
                    api_url="http://anythingllm",
                    api_key="key",  # pragma: allowlist secret -- synthetic upload fixture
                    workspace_slug="workspace",
                    run_root=root / "run",
                )

        submitted = [row["filename"] for row in captured["upload_plan_rows"]]
        self.assertEqual(len(submitted), 2)
        self.assertEqual(len({name.casefold() for name in submitted}), 2)
        self.assertTrue(all(name.startswith("shared-p001-s00001-d") for name in submitted))
    ...
    def test_grouped_upload_preserves_ocr_hold_and_submits_eligible_pdf(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            plan_file = root / "ready-upload-plan.csv"
            text_file = root / "ready.txt"
            text_file.write_text("ready", encoding="utf-8")
            plan_file.write_text(
                "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                f"ready.txt,Ready,,,local-pdf://sha256/ready,ready-p001,{text_file}\n",
                encoding="utf-8",
            )
            ready = {
                "pdf": str(root / "ready.pdf"),
                "source_sha256": "ready",
                "native_upload_plan": str(plan_file),
                "native_upload_transport": "file_upload",
            }
    ...
    def test_grouped_upload_keeps_custom_record_scope_and_identity_verifier(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for number in range(1, 4):
                text_file = root / f"page-{number}.txt"
                text_file.write_text(f"page {number}", encoding="utf-8")
                text_files.append(text_file)
            plan_file = root / "range-upload-plan.csv"
            plan_file.write_text(
                "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                + "".join(
                    f"page-{number}.txt,Page {number},,,local-pdf://sha256/range,range-p{number:03d},{text_files[number - 1]}\n"
                    for number in range(1, 4)
                ),
                encoding="utf-8",
            )
            summary = {
                "pdf": str(root / "range.pdf"),
                "source_sha256": "range",
                "native_upload_plan": str(plan_file),
                "native_upload_transport": "file_upload",
                "batch_upload_indices": [2],
            }
            captured = {}
            ...
            def fake_upload(*_args, **kwargs):
                ...
                verification = kwargs["batch_verifier"]({
                    "start_index": 0,
                    "end_index": 1,
                    "locations": ["custom-documents/page-2.txt.json"],
                })
                return {
                    "status": "complete", "uploaded": 1, "embedded": 1,
                    "attachment_results": [{
                        "source_path": row["_automatic_source_path"], "chunk_source": row["chunkSource"],
                        "location": "custom-documents/page-2.txt.json", "status": "attached", "error": "",
                    }],
                    "embedding_update": {"requested": 1, "accepted": 1, "batches": [{
                        "submission_state": "accepted", "searchability_proven": True,
                        "locations": ["custom-documents/page-2.txt.json"], "verification": verification,
                    }]},
                }

            verifier_calls = []
            def fake_verify(_storage, _workspace, source_sha, payloads, **kwargs):
                verifier_calls.append((source_sha, payloads, kwargs))
                return {
                    "status": "pass", "matching_vector_rows": 1,
                    "identity_set_checked": True,
                    "identity_set_complete": True,
                    "current_upload_vector_evidence_complete": True,
                    "current_upload_document_vector_count": 1,
                    "observed_chunk_sources": [payloads[0]["metadata"]["chunkSource"]],
                }
        ...
        self.assertEqual(len(captured["upload_plan_rows"]), 1)
        self.assertEqual(captured["upload_plan_rows"][0]["chunkSource"], "range-p002")
        self.assertEqual(verifier_calls[0][0], "")
        self.assertEqual(verifier_calls[0][1][0]["metadata"]["chunkSource"], "range-p002")
        # Live reconciliation uses the exact fast identity/count observation
        # path; the final source transaction audit remains the durable exact
        # proof. This must not regress to the older full storage scan per poll.
    ...
    def test_grouped_upload_outcome_rewrites_staged_per_document_reports(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = {
                ...
                "queue_requested": 2,
                "queue_accepted": 2,
                "queue_batches": [{"batch": 1, "requested": 2, "accepted": 2}],
                "ledger_path": str(Path(tmpdir) / "batch-embedding-ledger.json"),
            }
            app.persist_grouped_upload_outcome(summary, result)
            run_summary = json.loads((output / "run-summary.json").read_text(encoding="utf-8"))
            api_report = json.loads((output / "inspection" / "api-upload-report.json").read_text(encoding="utf-8"))
            post_report = json.loads((output / "inspection" / "post-upload-verification.json").read_text(encoding="utf-8"))
    ...
    def test_grouped_upload_attributes_attachment_error_to_its_pdf(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for name in ("first", "second"):
                ...
                text_file.write_text(name, encoding="utf-8")
                plan_file = root / f"{name}-upload-plan.csv"
                plan_file.write_text(
                    "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                    f"{name}.txt,{name},,,local-pdf://sha256/{name},{name}-p001,{text_file}\n",
                    encoding="utf-8",
                )
                summaries.append({
                    "pdf": str(root / f"{name}.pdf"), "source_sha256": name,
                    "native_upload_plan": str(plan_file), "native_upload_transport": "file_upload",
                })

            def fake_upload(*_args, **kwargs):
                rows = kwargs["upload_plan_rows"]
                return {
                    "status": "error", "uploaded": 1, "embedded": 0,
                    "attachment_results": [
                        {"source_path": rows[0]["_automatic_source_path"], "chunk_source": rows[0]["chunkSource"],
                         "location": "custom-documents/first.json", "status": "attached", "error": ""},
                        {"source_path": rows[1]["_automatic_source_path"], "chunk_source": rows[1]["chunkSource"],
                         "location": "", "status": "rejected", "error": "HTTP 500"},
                    ],
                    "embedding_update": {"requested": 0, "accepted": 0, "batches": [], "errors": []},
                    "errors": [{"classification": "attachment_batch_incomplete", "error": "No partial workspace queue was submitted."}],
                }

            with mock.patch.object(app, "maybe_upload_to_anythingllm", side_effect=fake_upload):
                report = app.upload_prepared_automatic_batch(
                    summaries, api_url="http://anythingllm", api_key="key",  # pragma: allowlist secret -- synthetic upload fixture
                    workspace_slug="workspace", run_root=root / "run",
                )

        first = report["document_results"][str(root / "first.pdf")]
        second = report["document_results"][str(root / "second.pdf")]
        self.assertEqual(first["uploaded"], 1)
        self.assertEqual(first["embedded"], 0)
        self.assertEqual(second["uploaded"], 0)
        self.assertEqual(second["error"], "HTTP 500")

    def test_missing_upload_plan_is_a_controlled_batch_error_not_directory_open(self):
        import rag_pdf_gradio_app as app

        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            summary = {
                "pdf": str(root / "missing-plan.pdf"),
    ...
    def test_invalid_upload_plan_row_blocks_the_entire_batch_before_upload(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            plan_file = root / "invalid-upload-plan.csv"
            # The plan is nonempty but lacks the required prepared text file.
            plan_file.write_text(
                "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                "missing.txt,Missing,,,local-pdf://sha256/missing,missing-p001,\n",
                encoding="utf-8",
            )
            summary = {
                "pdf": str(root / "missing-text.pdf"),
                "source_sha256": "missing",
                "native_upload_plan": str(plan_file),
                "native_upload_transport": "file_upload",
            }
    ...
    def test_grouped_upload_rejects_a_plan_filename_that_is_a_path(self):
        import rag_pdf_gradio_app as app
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            text_file = root / "prepared.txt"
            text_file.write_text("prepared", encoding="utf-8")
            plan_file = root / "unsafe-upload-plan.csv"
            plan_file.write_text(
                "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                f"..\\outside.txt,Unsafe,,,local-pdf://sha256/unsafe,unsafe-p001,{text_file}\n",
                encoding="utf-8",
            )
            summary = {
                "pdf": str(root / "unsafe.pdf"),
    ...
    def test_grouped_upload_rejects_an_empty_prepared_text_file_before_submission(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            text_file = root / "empty.txt"
            text_file.write_text("\n\t", encoding="utf-8")
            plan_file = root / "empty-upload-plan.csv"
            plan_file.write_text(
                "filename,title,docAuthor,description,docSource,chunkSource,text_file\n"
                f"empty.txt,Empty,,,local-pdf://sha256/empty,empty-p001,{text_file}\n",
                encoding="utf-8",
            )
            summary = {
                "pdf": str(root / "empty.pdf"),
                "source_sha256": "empty",
                "native_upload_plan": str(plan_file),
                "native_upload_transport": "file_upload",
            }
...
class PipelinePdfIntegrationTests(unittest.TestCase):
    ...
    def test_prepare_pdf_writes_clean_primary_fallback_and_reports(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # A deliberate final-page override is more specific than the
            # inclusive back-matter preference.  It must never be accepted by
            # the form then silently discarded by the preparation pipeline.
            args.end_page_override = 4
            output_with_explicit_end = root / "output-with-explicit-end"
            summary_with_explicit_end = pipeline.prepare_pdf(
                pdf_path, output_with_explicit_end, args
            )
            profile_with_explicit_end = json.loads(
                (output_with_explicit_end / "source-profile.json").read_text(encoding="utf-8")
            )
            self.assertEqual(summary_with_explicit_end["start_page"], 2)
            self.assertEqual(summary_with_explicit_end["end_page"], 4)
            self.assertEqual(summary_with_explicit_end["page_parents"], 2)
            self.assertEqual(
                profile_with_explicit_end["boundary_decisions"]["end_matter"]["excluded_end_range"],
                "4-12",
            )

    def test_deferred_lean_retention_keeps_raw_text_batch_upload_plan_until_shared_submission(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            pdf_path = root / "batch-source.pdf"
            doc = fitz.open()
            page = doc.new_page()
            page.insert_textbox(
                fitz.Rect(60, 60, 540, 760),
                "Batch source\n\n" + ("Distinctive text retained for the shared upload plan. " * 35),
                fontsize=10,
            )
            doc.save(pdf_path)
            doc.close()
            ...
            summary = pipeline.prepare_pdf(pdf_path, root / "output", args)
            plan = Path(summary["native_upload_plan"])
            plan_exists = plan.is_file()
            plan_rows = pipeline.load_upload_plan_rows(plan)
            page_parent_files_exist = (plan.parent / "file-upload-page-parents-native-header").is_dir()

        self.assertTrue(plan_exists)
        self.assertTrue(plan_rows)
        self.assertTrue(page_parent_files_exist)
        self.assertEqual(summary["lean_retention"], {
            "applied": False,
            "deferred": True,
...
class TimingAndInspectionSafetyTests(unittest.TestCase):
    ...
    def test_stale_picker_state_does_not_reject_a_visible_confirm_payload(self):
        import rag_pdf_gradio_app as app

        values = [None] * len(app.AUTOMATIC_RUN_FIELDS)
        values[app.AUTOMATIC_RUN_FIELDS.index("pdf_files")] = ["C:/selected.pdf"]
        delegated = object()

        def fake_stream(*_args, **_kwargs):
            yield delegated

        with mock.patch.object(app, "run_automatic_from_confirmation_stream", fake_stream):
            result = list(app.run_automatic_from_ready_confirmation_stream(
                {"state": "pending"}, *values
            ))

        self.assertEqual(result, [delegated])

    def test_stale_picker_state_without_a_pdf_still_does_not_start(self):
        import rag_pdf_gradio_app as app

        values = [None] * len(app.AUTOMATIC_RUN_FIELDS)
        self.assertFalse(app.automatic_confirmation_payload_has_selected_pdf(values))

    def test_exact_cache_reuse_marks_the_timing_text_without_claiming_a_lookup(self):
        import rag_pdf_gradio_app as app

        plain = app.automatic_run_timing_html(expected_seconds=90, state="running")
        cached = app.automatic_run_timing_html(
            expected_seconds=90,
            state="running",
            cache_reuse_records=7,
        )

        self.assertNotIn("cache-reuse-confirmed", plain)
        self.assertIn("cache-reuse-confirmed", cached)

    def test_opening_eta_optimism_is_presentation_only_and_cache_exempts_it(self):
        import rag_pdf_gradio_app as app

        self.assertEqual(app.opening_eta_presentation_seconds(100), 70)
        self.assertEqual(app.opening_eta_presentation_seconds(100, exact_cache_reuse_records=1), 100)
        rendered = app.automatic_run_timing_html(
            expected_seconds=100,
            presentation_expected_seconds=70,
            state="ready",
        )
        self.assertIn('data-model-expected-seconds="100"', rendered)
        self.assertIn("Est: 01m10s", rendered)
