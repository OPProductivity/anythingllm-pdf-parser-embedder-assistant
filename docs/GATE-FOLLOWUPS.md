...
| --- | --- | --- | --- |
| GATE-FOLLOWUP-001 | SQLite compatibility profiles fail closed for native mutation capability checks. | Add an API-only capability-discovery fallback for new Desktop schemas, with captured endpoint evidence. | Keep fail-closed for SQLite writes. |
